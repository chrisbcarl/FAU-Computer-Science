...
Due Time: 23:59
Total Points: 100
Assignment 8: Call Cost Calculator w/ File I/O Streams and Dynamic Arrays
...
1.	initialize is a void function  that has two output formal parameters; a dynamic array of call records called “call_DB” and an integer called “count”. Remember, count is the number of records that are being used in the array and it also represents the next available cell in the array where a new item can be stored. The function will read the data from the file “callstats_data.txt” into the dynamic array call_DB.  If call_DB becomes full, the function should call the function “double_size” to double the size (capacity) of call_DB.
...
3.	is_full is a Boolean function that has two integer input parameters, size and count. If count == size then true is return; otherwise false. The size is the capacity which is the total number of cells allocated to call_DB.
4.	search is an integer function that has three formal parameters. The array of records, call_DB; the input parameter count; the input parameter key which is the cell phone for the record you are searching for. The function will return the location of key in call_DB if it is there; otherwise -1 is returned.
5.	add inserts key (cell number) into call_DB.  Duplicates cell numbers are ok; add will prompt the user for the firstname, lastname, relays and call length. You may call process record to re-process when you add a new record.
6.	remove deletes all records with the cell number stored in key. If duplicate records exist with the same cell number they must all be deleted.
...
8.	process has two input parameters: call_DB and count. The function  process will calculate the net cost of a call (net_cost), the tax on a call (call_tax) and the total cost of the call (total_cost) using the number of relay stations (relays) and the length in minutes of the call (call_length) for all call records stored in call_DB.  Please consider the following:
...
9.	print has two output parameters, call_DB and count. The function will print every field of every call_record in call_DB to the screen.
10.	destroy_call_DB has one output parameter call_DB. This function will de-allocate all memory allocated to call_DB.  This should be the last function to be called before the program is exited.


LOG:
16.06.29, 14:19 - start
