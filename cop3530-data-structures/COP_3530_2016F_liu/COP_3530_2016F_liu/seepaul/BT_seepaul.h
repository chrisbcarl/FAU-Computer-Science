...
public:
	...
	BTree();	//constructors and destructor
	~BTree();	// initialize BTree_2 with the root r. r is the only node.
	BTree(Node *); // this one initializes one with a root // copy constructor
	BTree(BTree &); //this is acopy constructor, but the problem is that trees are recursive
					...
					//the function converts the array representation into a linked-list representation, i.e. BTree_2
	...
	void add2left(BTree *, const int &); //***not implemented
	void add2right(BTree *, const int &); //***not implemented
	...
	int * to_array(); //not implemented // convert the BT into the array form. Determine what the size n should be, and new an array of size n, and store the empty symbol at array[0]. Convert the BT into the array form and  retrun the array
	...
	friend bool isValidBTree(const int *p, const int n);	//checks if the size n is bigger than 1, checks that the first element is garbage, check that n is a 2==2^k, some kind of exponent of 2, declared as friend so that other functions also have access to it and we don’t need to put it above the class
...
//inorder copying?
void BTree::copyTree(Node * & lhs, Node * & rhs)
{
	if (rhs == 0)
	{
		lhs = 0;
...
//NOT PROPER YET
BTree::BTree(const int * data, const int & n)
{
	if (n == 0)
	{
...
void BTree::add2left(Node *newnode, Node *pos)
{
	if (newnode != 0)
		if (newnode->left == 0)
		{
...
void BTree::add2right(Node *newnode, Node *pos)
{
	if (newnode != 0)
		if (newnode->right == 0)
		{
...
void BTree::removeleaf(Node * pos)
{
	size--;
...
//swap is fine
void BTree::swapnodes(Node * n1, Node * n2)
{
	int temp = n1->data;
	n1->data = n2->data;
	n2->data = temp;
...
void BTree::print_pre_order(ostream & out)
{
	Node * temp = root;
	...
	else
	{
		...
		//recurse left
		BTree left = BTree(temp->left);
		left.print_pre_order(out);

		//recurse right
		BTree right = BTree(temp->right);
		right.print_pre_order(out);
	}

}
void BTree::print_in_order(ostream & out)
{
	Node * temp = root;
...
void BTree::print_post_order(ostream & out)
{
	Node * temp = root;
...
bool isValidBTree(const int *p, const int n)
{
...
int BT_seepaul()
{
	...
	*/
	...
	cout << "BTree1" << "\t" << "size == " << BTree1.size << endl;
	BTree1.print_pre_order(cout);
	BTree1.print_in_order(cout << endl);
	BTree1.print_post_order(cout << endl);
	cout << endl << endl;

	BTree BTree2 = BTree1;
	cout << "BTree2" << "\t" << "size == " << BTree2.size << endl;
	BTree2.print_pre_order(cout);
	BTree2.print_in_order(cout << endl);
	BTree2.print_post_order(cout << endl);
	cout << endl << endl;

	int input[16] = { -1, 8, 4, 12, 2, 6, 10, 14, 1, 3, 5, 7, 9, 11, 13, 15 };

	BTree BTree3 = BTree(input, 16);
	cout << "BTree3" << "\t" << "size == " << BTree3.size << endl;
	BTree3.print_pre_order(cout);
	BTree3.print_in_order(cout << endl);
	BTree3.print_post_order(cout << endl);
	cout << endl << endl;
